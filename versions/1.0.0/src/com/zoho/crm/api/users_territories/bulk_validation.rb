  ...
  module UsersTerritories
    ...
    class BulkValidation
      ...
      def alert
        ...
        # @param alert [Boolean] A Boolean

      def alert=(alert)
        if alert!=nil and ! [true, false].include?alert
          raise SDKException.new(Constants::DATA_TYPE_ERROR, 'KEY: alert EXPECTED TYPE: Boolean', nil, nil)
        end
        @alert = alert
        @key_modified['alert'] = 1
      end
